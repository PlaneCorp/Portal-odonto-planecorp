#!/usr/bin/env bash
# Regenera index.html embutindo fontes PlaneCorp + logo OdontoPrev em base64.
set -e
...
my $op=do{local(@ARGV,$/)=($ARGV[4]);<>};
$pc=~s/\s+$//; $al=~s/\s+$//; $op=~s/\s+$//;
my $h=do{local(@ARGV,$/)=($ARGV[2]);<>};
$h=~s/\@\@ALDOPC_B64\@\@/$pc/g;
$h=~s/\@\@ALDO_B64\@\@/$al/g;
$h=~s/\@\@ODONTO_B64\@\@/$op/g;
open(my $o,">",$ARGV[3]) or die $!; print $o $h; close $o;
my $left = ($h=~/\@\@(ALDO|ODONTO)/)?"FALHOU (tokens restantes)":"OK";
print "build odontoprev: $left\n";
' "$ASSETS/AldoPlaneCorp-base64.txt" "$ASSETS/Aldo-base64.txt" "$DIR/_template.html" "$DIR/../index.html" "$DIR/odonto-b64.txt"
